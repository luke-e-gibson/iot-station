...
const someMathExpression = (x: number) => (15
    - 11.25 * x
    + 51.875 * Math.pow(x, 2)
    - 33.125 * Math.pow(x, 3)
    + 8.125 * Math.pow(x, 4)
...
function timeframeToCount(timeframe: string): number {
    if (timeframe === "all") return -1 // Arbitrary large number to fetch all records
    const count = parseInt(timeframe, 10)
    return isNaN(count) ? 50 : count // Default to 50 if parsing fails
}
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const parsedTimeframe = timeframeToCount(timeframe)

                if (parsedTimeframe !== -1) {
                    if (selectedDevice === "all") {
                        const [latestData, statsData] = await Promise.all([
                            api.getLatestWeatherRecords(parsedTimeframe),
                            api.getWeatherStats(),
                        ])
                        if (!active) return
                        setData(latestData)
                        ...
                        const latestData = await api.getDeviceLatestWeatherRecords(selectedDevice, parsedTimeframe)
                        if (!active) return
                        setData(latestData)
                        setStats(computeStats(latestData))
                    }
                } else { // Unlimited timeframe - fetch all records for selected device(s)
                    if (selectedDevice === "all") {
                        const allData = await api.getAllWeatherRecords()
                        if (!active) return
                        setData(allData)
                        setStats(computeStats(allData))
                    } else {
                        const deviceData = await api.getDeviceWeatherRecords(selectedDevice)
                        if (!active) return
                        setData(deviceData)
                        setStats(computeStats(deviceData))
                    }
                }
            ...
            <div className="mx-auto max-w-6xl space-y-8">
                <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-end">
                        ...
                        <div className="flex w-full flex-col gap-1 sm:w-auto">
                            <span className="text-xs font-semibold uppercase text-muted-foreground">Data Points</span>
                            <Select value={timeframe} onValueChange={setTimeframe}>
                                ...
                                <SelectContent>
                                    ...
                                    {Array.from({length: 5}, (_, i) => someMathExpression(i + 1)).map((value) => (
